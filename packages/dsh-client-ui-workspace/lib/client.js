	...
	factory: (require) => {
		...
		*/
		function sessionVisible(session, current, archived) {
			return session.origin !== "subagent" && session.temporary !== true && !archived.has(session.id) && (!session.blank || session.id === current);
		}
		/**
		...
		*/
		function WorkspaceBrowser({ wide, expandSidebar, useSessions, useWorkspaces, useStore, actions, startSession, open, renameSession, forkSession, renameWorkspace, deleteWorkspace, insertWorkspaceBefore, archiveSession, insertSessionBefore, createWorkspace, searchSessions, searchResultLimit, useDirectoryFlow, renderSlot, t }) {
			...
			const onSessionDelete = (sessionId) => {
				...
				fetch("/_dsh/session-delete/api/delete", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: sessionId, confirmation: "DELETE " + sessionId }) }).then((response) => {
					const contentType = response.headers.get("content-type") ?? "";
					if (contentType.indexOf("application/json") !== 0) throw new Error("服务器未加载会话删除插件（请重启 DSH Web 后重试）");
					return response.json().then((data) => {
						if (!response.ok) throw new Error(data.error ?? ("HTTP " + response.status));
						if (data.deleted !== true) throw new Error("删除未完成");
						window.location.reload();
					});
				}).catch((reason) => { window.alert("删除失败：" + (reason && reason.message ? reason.message : reason)); });
			};
